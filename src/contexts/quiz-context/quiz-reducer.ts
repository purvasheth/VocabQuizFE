...
export function quizReducer(state: State, action: Action): typeof initialState {
  ...
  switch (action.type) {
    ...
    case VALIDATE_ANSWER:
      if (action.payload.selectedAnswer === words[currentWordIndex].mcq.answer) {
        return {
          ...state,
          ...
          words: words.map(
            (word, i) => (i === currentWordIndex ? { ...word, isBookmarked: false } : word),
          ),
        };
      }
      return {
        ...
        selectedAnswer: action.payload.selectedAnswer,
        words: words.map(
          (word, i) => (i === currentWordIndex ? { ...word, isBookmarked: true } : word),
        ),
      };
    ...
    case TOGGLE_IS_BOOKMARKED:
      return {
        ...state,
        words: words.map((word) => (word._id === action.payload.wordId
